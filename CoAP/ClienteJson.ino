...
void setup() {
    ...
    conectarWiFi();
    coap.start();
    Serial.println("Cliente JSON listo. Enviando solo JSON...");
...
void loop() {
    ...
    if (millis() - ultimo_envio >= INTERVALO_ENVIO) {
        ...
        char buffer[256];
        size_t tam = serializeJson(doc, buffer);

        coap.send(IP_SERVIDOR, PUERTO_COAP, RECURSO,
                  COAP_NONCON, COAP_POST, NULL, 0,
                  (uint8_t*)buffer, tam, COAP_APPLICATION_JSON);

        Serial.print("[JSON] Tamanio: ");
        Serial.print(tam);
        Serial.print(" B | ");
        Serial.print(buffer);
        Serial.println();
